...
number is produced rather than asserted, and where the rest of the machinery — the exhaustive
threshold, pins, exclusions, gating and the budget — is shown on the same payload.
...
{ "variants": [
  ...
  { "id": "response.body.items#cardinality=min+1;response.body.shippedAt#presence=absent;response.body.status#value=SHIPPED",
    ...
    "assignment": [ { "dimension": "response.body.items#cardinality", "point": "min+1" },
                    ...
                    { "dimension": "response.body.shippedAt#presence", "point": "absent" },
                    { "dimension": "response.body.status#value", "point": "SHIPPED" } ] },
  { "id": "response.body.shippedAt#presence=absent;response.body.status#value=DELIVERED",
    "label": "shippedAt=absent;status=DELIVERED", "origin": "covering",
    "assignment": [ { "dimension": "response.body.items#cardinality", "point": "min" },
                    { "dimension": "response.body.payment#alternative", "point": "card" },
                    { "dimension": "response.body.shippedAt#presence", "point": "absent" },
                    { "dimension": "response.body.status#value", "point": "DELIVERED" } ] },
  { "id": "response.body.items#cardinality=min+1;response.body.payment#alternative=invoice;response.body.status#value=DELIVERED",
    "label": "items=min+1;payment=invoice;status=DELIVERED", "origin": "covering",
    "assignment": [ { "dimension": "response.body.items#cardinality", "point": "min+1" },
                    { "dimension": "response.body.payment#alternative", "point": "invoice" },
                    { "dimension": "response.body.shippedAt#presence", "point": "present" },
                    { "dimension": "response.body.status#value", "point": "DELIVERED" } ] } ],
  "report": {
    ...
    "strategy": "t-wise", "strength": 2, "algorithm": "janus-ipog-v1", "selected": 6,
...
| 2 | min+1 | invoice | absent | PENDING | covering |
| 3 | min | invoice | present | SHIPPED | covering |
| 4 | min+1 | card | absent | SHIPPED | covering |
| 5 | min | card | absent | DELIVERED | covering |
| 6 | min+1 | invoice | present | DELIVERED | covering |

Six variants, 30 of 30 pairs. Every `status` appears with every `payment`, with both `items`
cardinalities and with both `shippedAt` presences; the same holds for every other pair of columns.
What is *not* covered is any particular triple — `(min, invoice, absent)` never runs — and that is the
strength-2 bargain, stated rather than hidden.
...
- **`status` was ordered first** by step 2 — it has three points, the others two — so the initial block
  is `status × items` and the sample is 6, the size of the largest single pair set. Ordering the
  dimensions as the shape emits them instead yields 7 variants for the same coverage.
- **Ids omit defaults.** Row 3 is `payment#alternative=invoice;status#value=SHIPPED`; its `items` and
  `shippedAt` are at their defaults and say nothing. Adding a fifth dimension to the shape leaves all
  six ids unchanged (spec §2.2).
...
`exhaustive` and all four variants run. Pairwise would also have selected 4 here: at two dimensions,
"every pair" *is* the whole space (spec §3.2's table). The threshold matters at three dimensions and
up, where 8 variants becomes 4, and it stops mattering at 12, where the saving is large enough that
sampling is the point.
...
defaults, giving `(min, card, absent, SHIPPED)` as row 2. The selection grows to **7** variants, still
covering 30 of 30 pairs — the pin is a seed, so the covering step works around it rather than
duplicating it. A pin never shrinks coverage and rarely costs more than the one variant it names.
...
The selection stays at **6** variants and the coverage report changes to `targets: 29, covered: 29,
removed: 1`: the excluded pair is not counted as a miss, because it is not a target. Row 4 of §2 —
...
is not a variant. The selection is **9** variants covering all 46. Three of them are `card` variants in
which `dueDate` is inactive and therefore has no point at all in the assignment — not `null`, not
`"absent"`, absent from the list (spec §2.1).
...
`serve-variant` takes the id, never the label. If the host runs five of the six and calls `finalise`,
the sixth is reported `not-exercised` and no pact comes back (protocol spec §8.2, spec §4.2) — the
