...
| `literal`, `one-of`, `given`, `request` (headers/query maps) | a `Map` with no defined encounter order (`Map.of`, `HashMap`, …) is written in **key order**, not "the order written"; a `LinkedHashMap`, a `SortedMap`, or `Shapes.map(...)` keeps its own order | Java has no ordered map literal. `Map.of` discards the written order, and its iteration order is salted per JVM run, so honouring it would make the same DSL produce different documents run to run. Key order is the only deterministic choice left. It only changes member order, never content. (The engine re-sorts members when it writes the contract anyway; see the report.) |
...
| `request`, `response` | header and query values must be a `String`, a `List<String>` or a shape helper's result. Anything else (e.g. an `int`) is refused, not stringified. Declaring the same header twice, in any case, is refused | the spec names only those three forms and forbids coercion. Lower-casing header names can make two declarations collide, and silently keeping one would lose the other |
